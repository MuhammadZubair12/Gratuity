...
def execute(filters=None):
	...
	for ss in salary_slips:
		...
		for e in earning_types:
			# frappe.msgprint(e)
			if e == "الراتب الأساسي" or e == "الراتب الأساسي للتشغيل":
				if frappe.as_json(ss_earning_map.get(ss.name, {}).get(e)) == 'null':
					pass
				else:
					row.update({"basic_salary": ss_earning_map.get(ss.name, {}).get(e) })
			if e == "بدل السكن" or e == "بدل سكن للتشغيل":
				if frappe.as_json(ss_earning_map.get(ss.name, {}).get(e)) == 'null' or frappe.as_json(ss_earning_map.get(ss.name, {}).get(e)) == "":
					pass
				else:
					row.update({"housing_allowance": ss_earning_map.get(ss.name, {}).get(e) })
			if e == "بدلات أخرى" or e == "بدلات اخرى للتشغيل":
				if frappe.as_json(ss_earning_map.get(ss.name, {}).get(e)) == 'null':
					pass
				else:
					row.update({"other_allowance": ss_earning_map.get(ss.name, {}).get(e) })
			# frappe.msgprint(frappe.as_json(ss_earning_map.get(ss.name, {}).get(e)))
			# row.update({frappe.scrub(e): ss_earning_map.get(ss.name, {}).get(e)})
			# bs = frappe.db.sql(""" select gross_pay from `tabSalary Slip` where name=%s """,(ss.name))
			# row.update({"basic_salary": flt(bs[0][0])})
